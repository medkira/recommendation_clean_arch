...
export class SignUpController extends BaseController {
  ...
  async execute(httpRequest: SignUpController.Request): Promise<HttpResponse> {
    ...
    if (role === UserRole.OWNER) {
      idOrError = await this.signUpOwner.execute({
        ...httpRequest.body,
        image: httpRequest.files?.profileImage,
      } as SignUpOwnerInterface.Request);
    } else if (role === UserRole.NORMAL) {
      idOrError = await this.signUpNormalUser.execute({
        ...httpRequest.body,
        image: httpRequest.files?.profileImage,
      } as SignUpNormalUserInterface.Request);
    } else {
...
export namespace SignUpController {
  export type Request = HttpRequest<SignUpNormalUserInterface.Request | SignUpOwnerInterface.Request,undefined,undefined, undefined, { profileImage: File[] }>;
